...
class MappedModel(models.AbstractModel):
    ...
    def _get_ir_translated_fields(self):
        ...
        for full_field_name in self.translated_fields:
            ...
            res[full_field_name] = (
                ...
                .sudo()
                .search([("model", "=", record._name), ("name", "=", f_name)])
            )
        return res
    ...
    def _fetch_translations(self, gmc_action):
        ...
        for lang in (
            ...
            .search([("translatable", "=", True), ("code_iso", "!=", "eng")])
        ):
            for translated_record in self.with_context(lang=lang.lang_id.code):
                ...
                for (
                    full_field_name,
                    t_field,
                ) in translated_record._get_ir_translated_fields().items():
                    ...
                    if full_field_name.endswith(".value"):
                        ...
                        gmc_terms = english_record.mapped(
                            full_field_name.replace(".value", ".name")
                        )
                    ...
                    for i, english_val in enumerate(english_vals):
                        ...
                        if english_val and english_val == translated_vals[i]:
                            ...
                            if f_type == "selection":
                                ...
                                s_field = (
                                    lang.env["ir.model.fields.selection"]
                                    .sudo()
                                    .search(
                                        [
                                            ("field_id", "=", t_field.id),
    ...
    def _find_translation(
        self, field_name, gmc_term, gmc_action, gmc_data, gmc_english_data
    ):
        """
        ...
        gmc_data = flatten(
            map(lambda d: d.values() if isinstance(d, dict) else d, gmc_data)
        )
        gmc_english_data = flatten(
            map(lambda d: d.values() if isinstance(d, dict) else d, gmc_english_data)
        )
        try:
            ...
            _logger.error(
                'English term "%s" not found in data received from GMC: %s',
                gmc_term,
                str(gmc_english_data),
            )
            return gmc_term
